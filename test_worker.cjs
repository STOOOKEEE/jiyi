...
(async()=>{
 ...
 const data=await caches.open(config.id+'-data-v3');for(const path of paths)assert(await data.match(u(path)));
 ...
 assert.equal((await message('purge')).ok,true);assert.equal(stores.has(config.id+'-data-v3'),false);
